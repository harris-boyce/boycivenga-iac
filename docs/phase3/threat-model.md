...
- ✅ Cryptographically attesting all artifacts with SLSA provenance
- ✅ Ensuring artifacts originate from trusted CI/CD workflows
- ✅ Detecting artifact tampering through digest verification
- ✅ Creating an audit trail for all artifact generation

Phase 3 explicitly **does not** protect against:
- ❌ Malicious or compromised deployment workflows
- ❌ Unauthorized Terraform/UniFi operations at runtime
- ❌ Compromised infrastructure automation agents
- ❌ NetBox data poisoning or unauthorized modifications
...
|-----------|-------------|-----------|---------------------|
| **NetBox** | ✅ Trusted | Authoritative source of network intent; access-controlled via NetBox RBAC | API read-only token, NetBox internal access controls |
| **Render Pipeline** | ✅ Trusted (currently) | Runs in GitHub-hosted runners with immutable workflows; SLSA attestation enabled | GitHub OIDC identity, workflow immutability, attestation signing |
| **Generated Artifacts** | ✅ Trusted (with attestation) | SLSA provenance proves origin and integrity | SHA-256 digests, cryptographic signatures, GitHub Attestations API |
| **Terraform Runtime** | ❌ NOT Trusted | Runtime execution is not verified in Phase 3; deferred to future phases | Documentation-only requirement for attestation verification |
| **UniFi Controller** | ❌ NOT Trusted | Runtime application of configs is manual and unverified | Out of scope for Phase 3 |
| **Deployment Agents** | ❌ NOT Trusted | Future automation agents have not been secured | Not yet implemented |

### Trust Boundary Diagram

```
┌─────────────────────────────────────────────────────────────┐
│ TRUSTED ZONE (Phase 3)                                      │
│                                                              │
│  ┌──────────┐       ┌─────────────────┐                    │
│  │ NetBox   │──────▶│ Render Pipeline │                    │
...
└─────────────────────┼──────────────────────────────────────┘
                      │
                      │ Trust Boundary (Phase 3)
                      │
┌─────────────────────┼──────────────────────────────────────┐
│ UNTRUSTED ZONE (Phase 3)                                   │
│                     │                                       │
│                     ▼                                       │
│  ┌─────────────────────────────────┐                      │
│  │ Runtime Systems (NOT TRUSTED)   │                      │
...
|------------|------------------|---------------|
| NetBox access is restricted to authorized users | NetBox RBAC, authentication | ✅ Assumed (managed outside this repository) |
| API tokens are read-only for render pipeline | NetBox token permissions | ✅ Enforced (documented requirement) |
| NetBox data is validated before entry | NetBox data validation, human review | ✅ Assumed (managed outside this repository) |
| NetBox instance is protected from unauthorized modification | NetBox infrastructure security | ✅ Assumed (managed outside this repository) |
...
**Trust Status**: ✅ **Trusted** (with caveats)
...
| Assumption | Required Control | Current State |
|------------|------------------|---------------|
| GitHub Actions infrastructure is secure | GitHub platform security | ✅ Assumed (GitHub-managed) |
| Workflow files cannot be modified without review | Branch protection, required reviews | ✅ Recommended (repository settings) |
| GitHub OIDC tokens are issued correctly | GitHub OIDC service | ✅ Assumed (GitHub-managed) |
| Render scripts are not malicious | Code review, testing | ✅ Enforced (PR review process) |
| Dependencies are not compromised | Dependency pinning, security scanning | ⚠️ Partial (pinning recommended) |
...
|--------|-------------|--------|---------------------|
| **Unauthorized Deployment** | Attacker runs `terraform apply` with attested but unauthorized artifacts | Infrastructure changes applied without approval | ❌ None |
| **Artifact Substitution** | Attacker uses attested artifact from a different site or version | Wrong configuration applied to infrastructure | ❌ None |
| **Malicious Terraform Provider** | Compromised Terraform provider executes malicious code | Infrastructure compromise, data exfiltration | ❌ None |
| **UniFi Controller Compromise** | Attacker gains access to UniFi controller and applies malicious configs | Network misconfiguration, service disruption | ❌ None |
| **Stale Artifact Usage** | Old attested artifacts are applied to infrastructure | Infrastructure state drift, security vulnerabilities | ⚠️ Partial (attestation timestamps) |
| **Manual Config Changes** | Operator bypasses attested artifacts and manually modifies infrastructure | Untracked changes, no audit trail | ❌ None |

### Trust Boundary at Runtime

```
┌────────────────────────────────────────────────────────┐
│ Attested Artifact (TRUSTED)                           │
│  - Proven to originate from render pipeline           │
│  - SHA-256 digest verified                            │
│  - Commit SHA and timestamp available                 │
└────────────────┬───────────────────────────────────────┘
                 │
                 │ TRUST GAP (Phase 3)
                 │
                 ▼
┌────────────────────────────────────────────────────────┐
│ Runtime Execution (UNTRUSTED)                         │
│  - No verification of who runs Terraform               │
│  - No verification of which artifacts are used         │
│  - No enforcement of deployment policies               │
│  - No protection against manual overrides             │
└────────────────────────────────────────────────────────┘
```
...
|--------|-----------|---------------|
| **Artifact Tampering** | SHA-256 digest verification via SLSA attestation | ✅ High (cryptographic) |
| **Artifact Forgery** | Cryptographic signatures using GitHub OIDC identity | ✅ High (keyless signing via Sigstore) |
| **Unknown Artifact Origin** | SLSA provenance includes workflow, commit SHA, timestamp | ✅ High (complete audit trail) |
| **Workflow Execution Tampering** | Immutable GitHub Actions logs, workflow run IDs | ✅ Medium (depends on GitHub platform) |
| **Unauthorized Workflow Modifications** | Version control, code review (recommended) | ⚠️ Medium (depends on repository settings) |
| **Supply Chain Attacks (Pipeline)** | SLSA Level 2 attestation, workflow isolation | ⚠️ Medium (SLSA Level 3 planned) |
...
|-----------------|----------|------------------|---------------------------|
| **Artifact Integrity** | 🔴 High | ✅ Fully Covered | N/A (complete) |
| **Artifact Provenance** | 🔴 High | ✅ Fully Covered | N/A (complete) |
| **Pipeline Integrity** | 🟡 Medium | ⚠️ Partially Covered | High (SLSA Level 3) |
| **Runtime Authorization** | 🔴 High | ❌ Not Covered | High (Phase 4) |
| **Data Validation** | 🟡 Medium | ❌ Not Covered | Medium (Future) |
| **Configuration Drift** | 🟡 Medium | ❌ Not Covered | Medium (Phase 5) |
| **Secrets Management** | 🔴 High | ⚠️ Partially Covered | High (Ongoing) |
...
**Phase 3 Protection**:
- ✅ Attacker cannot forge attested artifacts (requires GitHub OIDC token)
- ✅ Attacker cannot modify existing attested artifacts without detection (digest verification)

**Phase 3 Limitations**:
- ❌ Attacker can still run `terraform apply` with valid attested artifacts
- ❌ No authorization check prevents unauthorized deployment
- ❌ No policy enforcement prevents use of inappropriate artifacts

**Impact**: 🔴 High (Infrastructure compromise possible)
...
**Phase 3 Protection**:
- ⚠️ Branch protection and code review (if enabled) prevent direct workflow modification
- ⚠️ All workflow changes are audited in Git history
- ✅ Modified workflow runs still produce attestations traceable to the modified commit SHA

**Phase 3 Limitations**:
- ❌ If attacker bypasses code review, they can generate malicious attested artifacts
- ❌ Attestations prove provenance but do not validate artifact content

**Impact**: 🔴 High (Malicious artifacts with valid attestations)
...
**Phase 3 Protection**:
- ✅ NetBox access controls and audit logs (external to this repository)
- ✅ Render pipeline uses read-only API token (cannot modify NetBox)

**Phase 3 Limitations**:
- ❌ No validation of NetBox data quality or safety
- ❌ Render pipeline trusts all NetBox data implicitly
- ❌ Malicious intent data results in malicious attested artifacts

**Impact**: 🔴 High (Malicious intent becomes malicious infrastructure)
...
**Phase 3 Protection**:
- ✅ Token is read-only (cannot modify NetBox)
- ✅ Token is stored as GitHub secret (encrypted at rest)

**Phase 3 Limitations**:
- ❌ Attacker can read all network intent data (information disclosure)
- ❌ Attacker could generate artifacts locally using stolen token

**Impact**: 🟡 Medium (Information disclosure, no direct infrastructure impact)
...
**Phase 3 Protection**:
- ⚠️ Attestations include timestamps (can detect staleness)
- ⚠️ Attestations include commit SHA (can trace to old code)

**Phase 3 Limitations**:
- ❌ No automatic freshness enforcement
- ❌ No policy preventing deployment of old artifacts

**Impact**: 🟡 Medium (Infrastructure drift, potential security vulnerabilities)
...
| Signed provenance | ✅ Keyless signing via Sigstore |
...
|-------------|--------|---------------|
| Hardened build platform | ❌ Not implemented | Future |
| Non-falsifiable provenance | ❌ Not implemented | Future (SLSA builder) |
| Isolation | ❌ Not implemented | Future |
